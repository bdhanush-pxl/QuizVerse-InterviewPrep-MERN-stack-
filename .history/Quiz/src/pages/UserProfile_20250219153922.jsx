...
// Move all helper functions to the top, outside of any component
const calculateStreak = (attempts) => {
  ...
  const lastAttemptDate = new Date(sortedAttempts[0].attemptDate);
  lastAttemptDate.setHours(0, 0, 0, 0);
  
  // If the last attempt is not from today or yesterday, streak is broken
  const daysSinceLastAttempt = Math.floor((today - lastAttemptDate) / (1000 * 60 * 60 * 24));
  if (daysSinceLastAttempt > 1) return 0;
  
  let streak = 1;
  let currentDate = lastAttemptDate;
  
  // Check consecutive days backwards
  for (let i = 1; i < sortedAttempts.length; i++) {
    const attemptDate = new Date(sortedAttempts[i].attemptDate);
    attemptDate.setHours(0, 0, 0, 0);
    
    // Calculate difference in days
    const dayDifference = Math.floor((currentDate - attemptDate) / (1000 * 60 * 60 * 24));

    if (dayDifference === 1) {
      streak++;
      currentDate = attemptDate;
    } else if (dayDifference === 0) {
      // Same day attempt, continue checking
      continue;
    } else {
      // Streak is broken
      break;
...
// Updated FeedbackForm with interactive star rating
const FeedbackForm = () => {
...
function UserProfile() {
  ...
  // Calculate scores once and store in variables
  ...
  useEffect(() => {
    ...
    setUserStats(prevStats => ({
      ...
      joinedDate: profileStats.joinedDate 
        ? new Date(profileStats.joinedDate).toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
          })
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const recentActivity = quizService.getRecentActivity();
      const achievements = JSON.parse(
        localStorage.getItem(`achievements_${localStorage.getItem('userId')}`) || '[]'
      );

      setUserStats(prev => ({
        ...prev,
        quizAttempts: recentActivity,
        achievements
      }));
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        if (data.status === 'ok') {
          // Get unique attempts per day for streak calculation
          const uniqueAttempts = getUniqueAttemptDays(data.quizAttempts || []);
          
          // Calculate streak using unique attempts
          const currentStreak = calculateStreak(uniqueAttempts);
          ...
          // Calculate achievements with correct streak
  ...
  // Enhanced achievement animation variants
  ...
  // Update the Performance Summary section in ActivityGraph component
  ...
  return (
    ...
    >
      <BackButton />
      {/* Profile Header - Updated glass morphism effect */}
      <motion.div 
        variants={itemVariants}

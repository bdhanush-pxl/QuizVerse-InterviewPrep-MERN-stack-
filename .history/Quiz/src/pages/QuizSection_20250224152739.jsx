...
const calculateStreak = (attempts) => {
  ...
  // If the latest attempt is not from today or yesterday, streak is broken
...
function QuizSection() {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (data.status === 'ok') {
          ...
          // Update the achievements count in the stats display
  ...
  const handleQuizStart = async (quiz, route) => {
    try {
      const response = await fetch('https://quiz-verse-interview-prep-mern-stack-fd2x.vercel.app/api/update-attempt-status', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const toggleSaveQuiz = async (quizId, quizTitle) => {
    try {
      ...
      const response = await fetch('https://quiz-verse-interview-prep-mern-stack-fd2x.vercel.app/api/save-quiz', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
